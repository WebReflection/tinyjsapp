...
| `{role:'edit', items}` — items under Select All (mac) / an "Edit" menu of just them (win, linux), in the declared slot; click, key, `menu.update`/`get` all reach them; bare `{role:'edit'}` still draws nothing off-mac | ⬜ | ✅ 2026-09-22 — seen by hand: items under Select All in the declared slot; `menu.get`/`update` reach them | ⬜ |
| stock roles in edit items (`{role:'standard'}` last, `{role:'copy'}` …) — order as declared; Copy greys out with nothing selected; an `enabled:false` item of ours stays grey after the menu opens (validateMenuItem via tag) | ⬜ roles dropped, separators tidied | ✅ 2026-09-22 — all 6 shapes seen by hand (order, Copy greyed, disabled item stays grey) + `menu.get` read back | ⬜ roles dropped, separators tidied |
